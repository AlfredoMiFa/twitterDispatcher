package com.twitter.dispatcher.domain.service
...
class TwitterDispatcher(private @Value("\${queue.twitter}") val queue:String,
    ...
    fun dispatch():Flux<Tweet> {
        return this.receiver.consumeAutoAck(this.queue).flatMap {
            message -> Mono.just(mapper.readValue<Tweet>(message.body))
        }
    }
